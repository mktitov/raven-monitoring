...
public class RrdUpdateQueueNode extends BaseNode
{
	...
	public void pushUpdateRequest(RrdUpdateRequest updateRequest)
	{
		if (getStatus()!=Status.STARTED)
		{
			error(String.format("Error handling write request. Node (%s) not started", getPath()));
			return;

		}
		if (isLogLevelEnabled(LogLevel.DEBUG))
			debug(String.format(
					"Recieved update request for rrd (%s). " +
					"Request info: time (%d), value (%s)"
					, updateRequest.getDatabasePath(), updateRequest.getTime()
		...
		executor.execute(new UpdateRequestExecutor(updateRequest));
	}
	...
	private class UpdateRequestExecutor implements Runnable
	{
		private final RrdUpdateRequest updateRequest;

		public UpdateRequestExecutor(RrdUpdateRequest updateRequest)
		{
		...
		public void run()
		{
			...
			try
			{
				RrdDb db = dbPool.requestRrdDb(updateRequest.getDatabasePath());
				try
				{
					Sample sample = db.createSample(updateRequest.getTime());
					sample.setValue(0, updateRequest.getValue());
					sample.update();
				}
				finally
				{
			...
			{
				error(String.format(
						"Error updating rrd (%s). %s"
						, updateRequest.getDatabasePath(), ex.getMessage())
					, ex);
			}
