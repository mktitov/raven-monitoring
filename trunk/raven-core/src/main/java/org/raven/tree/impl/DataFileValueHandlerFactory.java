 ...
 */
public class DataFileValueHandlerFactory implements AttributeValueHandlerFactory
{
    public final static String TYPE = "DataFile";
    private final NodePathResolver pathResolver;

    public DataFileValueHandlerFactory(NodePathResolver pathResolver)
    {
    ...
    public AttributeValueHandler createValueHandler(NodeAttribute attribute)
    {
        try
        {
            return new DataFileValueHandler(attribute);
        }
        catch (DataFileValueHandlerException ex)
        {
